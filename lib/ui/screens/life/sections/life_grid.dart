...
class _LifeGrid extends StatefulWidget {
  ...
  @override
  _LifeGridState createState() => _LifeGridState();
}
...
class _LifeGridState extends State<_LifeGrid> {
  ...
  LifeCardBloc get cardBloc => context.read<LifeCardBloc>();
  ...
  Future _onRefresh() async {
    ...
    return cardBloc.stream.firstWhere((e) => e.status != LifeCardStateStatus.loading);
  }

  void _oncardPress(mocard.Card card) {
    cardBloc.add(CardSelectChanged(cardId: card.id));

    AppNavigator.push(Routes.lifeInfo, card);
  }
  ...
  Widget build(BuildContext context) {
    return NestedScrollView(
      ...
      body: CardStateStatusSelector((status) {
        switch (status) {
          case LifeCardStateStatus.loading:
            return _buildLoading();

          case LifeCardStateStatus.loadSuccess:
          case LifeCardStateStatus.loadMoreSuccess:
          case LifeCardStateStatus.loadingMore:
            return _buildGrid();

          case LifeCardStateStatus.loadFailure:
          case LifeCardStateStatus.loadMoreFailure:
            return _buildError();
